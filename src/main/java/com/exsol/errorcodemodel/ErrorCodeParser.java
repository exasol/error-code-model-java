...
class ErrorCodeParser {
    private static final Pattern ERROR_CODE_PATTERN = Pattern
            .compile("(?:([^-]+)-)?([A-Z][A-Z0-9]{0,9}(?:-[A-Z][A-Z0-9]{0,9})?)-(\\d+)");
    ...
    public ErrorIdentifier parse(final String errorCodeString) throws ErrorIdentifier.SyntaxException {
        ...
        if (!matcher.matches()) {
            throw new ErrorIdentifier.SyntaxException(ExaError.messageBuilder("E-ECMOJ-1")
                    .message("The error code {{error code}} has an invalid format.")
                    .parameter("error code", errorCodeString)
                    .mitigation("Use a code like 'EXA-1', 'E-EXA-1' or 'W-EXA-MOD-2', tags can have max. 10 chars.").toString());
        }
